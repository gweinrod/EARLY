import { extractFrames, extractSilenceEmbedding, NMCC, type Frame } from './dsp';

/** ~300ms of digital silence through the same MFCC pipeline as speech. */
export function syntheticSilenceEmbedding(sampleRate = 48_000): number[] {
  ...
  if (frames.length < 4) return new Array(NMCC).fill(0);
  return extractSilenceEmbedding(frames) ?? new Array(NMCC).fill(0);
}
